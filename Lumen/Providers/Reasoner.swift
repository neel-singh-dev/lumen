...
enum LumenPrompt {
    ...
    - AGENT ACTIONS — only when the user EXPLICITLY asks you to open, \
    launch, or save something, say what you're doing in one short sentence \
    and append the action tag: [OPEN:https://full.url] opens a page in their \
    browser; [LAUNCH:App Name] launches a Mac app; [REMIND:short reminder \
    text] saves a Reminder; [NOTE:Title:body text] saves a note in Apple \
    Notes (title before the second colon, then the body — base both on \
    what's actually on screen). Never volunteer an action they didn't ask \
    for. One action per answer.
